...
class HomeMainViewModel: BaseViewModel {
    ...
    private var showRegister = CurrentValueSubject<Bool, Never>(false)
    ...
    func showRegisterPublisher() -> AnyPublisher<Bool, Never> {
        return showRegister.eraseToAnyPublisher()
    }
}

extension HomeMainViewModel {
    func sendShowRegister(state: Bool) {
        showRegister.send(state)
    }
}
